...
########################################################################
##
## MI function for imputing MNAR hierarchical ordered data (GHQ)
...
#' @export
mice.impute.2l.ordinalMNAR <- function(y, ry, x, type, QP=rep(10,2), draw=T, excl="",...) { 
  ...
  heckit <- loglik_ordbivprob_est_ghq(data=data,yS=ry, yO=y, XO=XO, XS=XS, kS=ind_s, kO=ind_y,sel = sel, out=out, group=group, QP=QP) 
  ...
  modes <- t(sapply(1:length(table(data$group)), function(x) { 
    stats::optim(par=start2, fn=mode_ord, gr=NULL, method="Nelder-Mead", x=x, yS=ry, yO=y, linpredS=linpredS_int, linpredO=linpredO_int, 
  ...
  hess_mode <- lapply(1:nrow(modes), function(x) { 
    maxLik::numericHessian(f=mode_ord, grad=NULL, t0=modes[x,], x=x, yS=ry, yO=y, linpredS=linpredS_int, linpredO=linpredO_int, group_aux=group_aux,
...
mice.impute.2l.ordinalMNAR  <- compiler::cmpfun(mice.impute.2l.ordinalMNAR)
